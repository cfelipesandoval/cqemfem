...
ordered 
"""
def main_TE(modes, fileName, order):
  print("Beginning TE mode FEM")

  # Element Initialization
  # order = int(input("Select integer basis order > 0: "))
  assert order > 0, "Error: Invalid basis order"
  assert order < 4, "Error: >3rd order bases not implemented"
  start_time = time.time()
  print("...Initializing elements...")
  elements, nodeList, boundaryList = initMesh(fileName, 9, order)
  ...
  for el in elements:
    for i in range(int((order + 1) * (order + 2) / 2)):
      for j in range(int((order + 1) * (order + 2) / 2)):
  ...
  # matDic = {"A_mat": A, "B_mat": B}
  # sc.io.savemat("second_order.mat", matDic)
  # Ek, Ev = sc.linalg.eig(A, b=B)
  E_eigVals = np.array(Ek)
  E_eigVecs = np.array(Ev)
  print(f"Eigen Done, time elapsed = {time.time() - start_time} s")

  eigVal_order = np.argsort(E_eigVals)
  E_eigVals = E_eigVals[eigVal_order]
  E_eigVecs = E_eigVecs[:,eigVal_order]
