...
def readAbq(fName):
    ...
    while currLine.find("**") == -1 and len(        # read out ELEMENTS
        ...
        if currLine.find("ELEMENT,") != -1:         # element property line catch
            ...
            if typeMat.find("epr") != -1:           # extracts material property
            ...
            if isBndry:                             # constructs [bndry]: first elm is a flag if 1D boundary
                ...
                elements.append([int(currLine[1]),  # node indices
                                 int(currLine[2]),
                                 int(currLine[3]),
                                 int(currLine[4]) 
                                 if elmDim else -1, # differentiate 2D/3D
                                 int(currLine[0]),  # Cubit elm num (needed for AMR)
                                 typeMat])          # material property
        currLine = file.readline()
...
def meshRefine(fName, cubitPath, elmsList, E_eigs, B_eigs):
    ...
    for idx in range(len(badElms)):
        elmType = "tet"
        cubit.cmd(f"refine {elmType} {badElms[idx]} depth 1 smooth")
        # tet/tri if needed
    ...
    cubit.cmd("save cub5 'C:/Users/dongg/Documents/Work/VIP279/amr2af2.cub5'")
...
def errorGen(elm):
    return 5 if elm == 10335 or elm == 10344 or elm == 10353 else 0